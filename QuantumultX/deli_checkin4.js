...
^https?:\/\/kq\.delicloud\.com\/attend\/.* url script-request-body https://raw.githubusercontent.com/ShawnBenC/shawnbenc/refs/heads/main/QuantumultX/deli_checkin4.js

[task_local]
# 早上、下午 (8点,13点的 45-55分 每分钟轮询)
45-55 8,13 * * 1-5 https://raw.githubusercontent.com/ShawnBenC/shawnbenc/refs/heads/main/QuantumultX/deli_checkin4.js, tag=得力打卡(早/下), enabled=true

# 下班时间段 (12点的 01-11分 每分钟轮询)
01-11 12 * * 1-5 https://raw.githubusercontent.com/ShawnBenC/shawnbenc/refs/heads/main/QuantumultX/deli_checkin4.js, tag=得力打卡(中午), enabled=true

# 下班时间段 (17点的 31-41分 每分钟轮询)
31-41 17 * * 1-5 https://raw.githubusercontent.com/ShawnBenC/shawnbenc/refs/heads/main/QuantumultX/deli_checkin4.js, tag=得力打卡(晚退), enabled=true
...
function doCheckin() {
    ...
    // 设计定位：纯人工完全控制，代打/补卡/测试专用。
    // 核心规则：
    //   1. 彻底跳过时间窗口检查
    //   2. 彻底跳过防重拦截检查（不查也不拦）
    //   3. 彻底跳过 10% 随机概率与保底（无需模拟自然打卡）
    //   4. 打卡成功后【绝对不写入防重锁定】，保持定时系统环境纯净
    // =========================================================================
    if (isForceRun) {
    ...
    // 定时轮询时间窗口定义（与 task_local 保持严格一致）
    ...
    // -------------------------------------------------------------------------
    // 情况 B2：时间窗口内（Cron 定时轮询任务）
    // -------------------------------------------------------------------------
    // 1. 防重核心拦截：若本班次已打过卡，立即退出，防止轮询重复打卡
    if (hasCheckedIn === "true") {
            ...
            if (error) {
                ...
                try {
                    ...
                    if (res.errno === 0 || res.errmsg === "ok") {
                        ...
                        // 只有提供了 flagKeyToLock (即非强制模式) 才写入防重标记！
                        if (flagKeyToLock) {
